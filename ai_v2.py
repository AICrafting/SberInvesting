# Используется модуль tensorflow.keras который позволяет обучить ИИ который может предсказать будущие цены акций -> минус - для того, чтобы были качетственные результаты надо очень долго ждать
import numpy as np
